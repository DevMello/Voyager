...
public class Scheduler {
    /**
     * The {@link Voyager} instance the scheduler is controlling.
     */
     ...
     *
     * @param voyager the {@link Voyager} instance that the scheduler
     *                   will act upon.
     */
    public Scheduler(Voyager voyager) {
    ...
    public void tick() {
        ...
        if (!currentTask.hasStarted()) {
            ...
            this.voyager.followTrajectory(currentTask.getTrajectory());
        } else {
            if (!(Math.abs(currentTask.getMinTimeMilliseconds()) > 1_000_000)) {
                ...
                PointXYZ currentRobotPosition =
                    this.voyager.getOdometry().getPosition();
