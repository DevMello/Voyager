...
public class MovementRecorder {
     ...
     *
     * @param voyager the instance of Pathfinder that's being utilized.
     ...
     *                   a larger file size.
     */
    public MovementRecorder(Voyager voyager, double minDelayMs) {
    ...
    public void tick() {
        ...
        if (elapsed >= minDelayMs) {
            MovementRecord record = new MovementRecord(
                voyager.getPosition(),
                voyager.getVelocityXY(),
                elapsed,
                voyager.getTranslation()
            );
