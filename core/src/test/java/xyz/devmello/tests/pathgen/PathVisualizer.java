...
public class PathVisualizer {
    ...
    public static void visualize(List<Zone> zones, List<PointXY> path, PointXY startPoint, PointXY endPoint, double minX, double maxX, double minY, double maxY) {
        ...
        // Draw wall around the boundaries
        for (int i = 0; i < TERMINAL_WIDTH; i++) {
            grid[0][i] = WALL_CHAR;
            grid[TERMINAL_HEIGHT - 1][i] = WALL_CHAR;
        }
        for (int i = 0; i < TERMINAL_HEIGHT; i++) {
            grid[i][0] = WALL_CHAR;
            grid[i][TERMINAL_WIDTH - 1] = WALL_CHAR;
        }
        ...
        for (Zone zone : zones) {
            ...
            if (shape instanceof Rectangle) {
                ...
                int startGridX = Math.max(0, (int) Math.round(mapValue(zoneMinX, minX, maxX, 0, TERMINAL_WIDTH - 1)));
                int endGridX = Math.min(TERMINAL_WIDTH - 1, (int) Math.round(mapValue(zoneMaxX, minX, maxX, 0, TERMINAL_WIDTH - 1)));
                int startGridY = Math.max(0, (int) Math.round(mapValue(zoneMinY, minY, maxY, 0, TERMINAL_HEIGHT - 1)));
                int endGridY = Math.min(TERMINAL_HEIGHT - 1, (int) Math.round(mapValue(zoneMaxY, minY, maxY, 0, TERMINAL_HEIGHT - 1)));
        ...
        if (path != null && path.size() > 0) {
            for (PointXY point : path) {
                int gridX = (int) Math.round(mapValue(point.x(), minX, maxX, 0, TERMINAL_WIDTH - 1));
                int gridY = (int) Math.round(mapValue(point.y(), minY, maxY, 0, TERMINAL_HEIGHT - 1));
        ...
        if (startPoint != null) {
            int startGridX = (int) Math.round(mapValue(startPoint.x(), minX, maxX, 0, TERMINAL_WIDTH - 1));
            int startGridY = (int) Math.round(mapValue(startPoint.y(), minY, maxY, 0, TERMINAL_HEIGHT - 1));
        ...
        if (endPoint != null) {
            int endGridX = (int) Math.round(mapValue(endPoint.x(), minX, maxX, 0, TERMINAL_WIDTH - 1));
            int endGridY = (int) Math.round(mapValue(endPoint.y(), minY, maxY, 0, TERMINAL_HEIGHT - 1));
        ...
        System.out.println("--------------------");
        System.out.println("S = Start, E = End, # = Path, X = Zone, * = Wall");
    }
